...
@router.get("/")
def get_genres(token: validate_token, db: db_session):
    return games_db.get_games_field(db, Genre)


@router.post("/", status_code=status.HTTP_201_CREATED)
def create_genre(token: validate_token, db: db_session, genre_info: GenreCreate):
...
@router.delete("/id/{id}")
def delete_genre_by_id(token: validate_token, db: db_session, id: UUID):
    deleted_genre = genre_db.delete_genre_by_id(db, id)
    return deleted_genre


@router.get("/id/{id}")
def get_genre_by_id(token: validate_token, db: db_session, id: UUID):
...
@router.patch("/")
def update_genre(token: validate_token, db: db_session, new_genre: GenreUpdate):
